...
if (isset($_GET['search'])) {
    ...
    $sql1 = "select bloodinfo.*, hospitals.* from bloodinfo, hospitals where bloodinfo.hid=hospitals.id && bg='$searchKey'";
} else {
    $sql = "SELECT blooddinfo.*, receivers.* from blooddinfo, receivers where blooddinfo.rid=receivers.id";
    $sql1 = "select bloodinfo.*, hospitals.* from bloodinfo, hospitals where bloodinfo.hid=hospitals.id";
}
...
<?php require 'head.php'; ?><style>
    <?php require 'navbarhos.php'; ?>body {
...
<body>
    ...
    <table class="table table-responsive table-striped rounded mb-5">
        ...
        <?php while ($row2 = mysqli_fetch_array($result1)) { ?>
            ...
            <tr>
                <td><?php echo ++$counter; ?></td>
                <td><?php echo $row2['hname']; ?></td>
                <td><?php echo ($row2['hcity']); ?></td>
                <td><?php echo ($row2['hemail']); ?></td>
                <td><?php echo ($row2['hphone']); ?></td>
